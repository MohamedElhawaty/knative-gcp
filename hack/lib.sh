...
readonly CONTROLLER_GSA="events-controller-gsa"
...
function init_service_account_with_pubsub_editor() {
  local project_id=${1}
  local service_account=${2}
  echo "parameter project_id used when initiating pubsub service account is'${project_id}'"
  echo "parameter service_account to be granted pubsub.editor role is'${service_account}'"
  # Enable APIs.
  gcloud services enable pubsub.googleapis.com
  ...
  if [ -z "${existing_pubsub_service_account}" ]; then
    echo "Create PubSub Service Account '${service_account}' neeeded for the Data Plane"
    gcloud iam service-accounts create "${service_account}"
  else
    echo "PubSub Service Account '${service_account}' needed for the Data Plane already existed"
  fi
